...
public final class StompWebSocketClientPageHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest request) throws Exception {
        ...
        if (request.decoderResult().isFailure()) {
            FullHttpResponse badReuqest = new DefaultFullHttpResponse(request.protocolVersion(), BAD_REQUEST);
            sendResponse(badReuqest, ctx, true);
            return;
        }
    ...
    private static boolean sendResource(FullHttpRequest request, ChannelHandlerContext ctx) {
        ...
        try {
            ...
            System.out.println("Cannot read file lenght " + io.getMessage());
            return false;
        } finally {
        ...
        if (requestResource.endsWith("html")) {
            contentType = "text/html; chaarset=UTF-8";
